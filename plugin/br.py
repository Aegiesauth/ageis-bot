...
async def broadcast_to_dialog(client, chat_id, text):
    try:
        await client.send_message(chat_id, text)
    except FloodWait as e:
        await asyncio.sleep(e.x)  # Sleep if Telegram gives a FloodWait error
...
async def broadcast_message(client, message):
    ...
    # Gather all chat IDs the bot is part of
    ...
    for i in range(0, len(chat_ids), batch_size):
        ...
        tasks = [broadcast_to_dialog(client, chat_id, text_to_broadcast) for chat_id in batch_chat_ids]
